...
const info = await page.evaluate(() => {
  ...
  out.push('submit aria=' + (submit ? submit.getAttribute('aria-label') : 'none') + ' (draft page)');
  const prev = document.querySelector('[data-testid=calendar-prev-btn]');
  out.push('cal prev aria=' + (document.querySelector('#/calendar') ? 'x' : 'n' + (prev ? prev.getAttribute('aria-label') : 'none')));
