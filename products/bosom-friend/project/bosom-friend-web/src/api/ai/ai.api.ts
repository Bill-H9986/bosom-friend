...
import type { CleanupRunningTasksResponse } from './ai.types'
import type { AssetListVo } from '@web/types/agent-asset'
...
export const agentApi = {
  ...
  async createTaskWithSSE(
    ...
    try {
      ...
      await fetchEventSource(url, {
        ...
        // 不再随请求发浏览器缓存的模型配置：服务端以 llm-user.json 为唯一权威，
        // 未带 llm 时自动回退到它（server/src/api.ts 的 parseLlmOverride(...) ?? readStoredLlm(...)）。
  ...
  /**
   * 清理运行中的任务（批量，用于清除历史卡死的"运行中"假状态）
   * @returns 后端返回的清理数量 updatedCount
   */
  async cleanupRunningTasks() {
    const res = await http.post<CleanupRunningTasksResponse>(
      'agent/tasks/cleanup-running',
      undefined,
      true, // 静默模式：不弹通用错误提示，由调用方自行处理
    )
    return res
  },
   ...
   * 批量删除任务
   *
   * 后端一条都没删掉时会回非零业务码（"这些任务不存在或已被删除"），
   * 调用方必须按返回值判定，不能一律提示成功。
   *
   * @param taskIds 要删除的任务 ID 列表
   * @returns 含 deleted（实际删除条数）的信封
   */
  async deleteTasks(taskIds: string[]) {
    const res = await http.delete<{ deleted: number }>('agent/tasks', { taskIds }, true)
    return res
  },
...
export async function aiChatStream(data: AiChatStreamParams) {
  ...
  // 全局大模型配置：激活自定义厂商后，模型与接口参数全系统生效
  const llmConfig = useLlmConfigStore.getState()
  const chatModel = llmConfig.getChatModel()
  const chatEndpoint = llmConfig.getChatEndpoint()

  const response = await fetch(`${WEB_API_BASE_URL}/ai/chat`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      ...
      ...(chatEndpoint && { customLlm: chatEndpoint }),
      ...data,
    }),
  })
...
/** 删除一条或多条生成记录 */
export function apiDeleteDraftGenerations(taskIds: string[]) {
  return http.delete('ai/draft-generation', { taskIds })
}

/** 清空全部生成记录 */
export function apiClearDraftGenerations() {
  return http.delete('ai/draft-generation', { all: true })
}
