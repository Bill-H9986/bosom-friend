import type { PublishStatus } from '../platforms/publish.constants'
import type { PlatformStatus, PublishContentMode } from './channel.constants'
import type { PlatType } from '@web/app/config/platConfig'
import type { PubType } from '@web/app/config/publishConfig'
...
export interface PlatformMetadataVo {
  ...
  /** 产品是否把它作为频道开放；引擎能发布但产品未接入的平台为 false/缺省。 */
  channel?: boolean
...
export interface PlatformInfo {
  ...
  platform: PlatType
  status: PlatformStatus
  /** 产品真实接入的频道平台；由服务端目录下发，前端不再自建白名单。 */
  channel: boolean
  name: string
  icon: string
  logoUrl: string
