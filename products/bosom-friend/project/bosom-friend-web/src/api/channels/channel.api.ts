 ...
 * 绑定成功后同步账号到服务端账号库，幂等
 */
export function browserRegisterAccountApi(platform: PlatType, account: { uid?: string; nickname?: string; avatar?: string; loginCookie?: string }) {
  return http.post<{ code: number; data?: { id: string } }>(
    'v2/channels/accounts/browser-register',
    { type: platform, uid: account.uid, nickname: account.nickname, avatar: account.avatar, loginCookie: account.loginCookie },
    true,
  )
}
