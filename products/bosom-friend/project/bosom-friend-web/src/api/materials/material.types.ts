...
import type { DraftGenerationRequest, ImageTextDraftType, VideoDraftType } from '@web/api/ai/ai.types'
 ...
 * 批量删除素材结果。
 *
 * 仍被生成记录引用的素材服务端会保留（避免生成记录断图），因此删除必须按实际结果回话：
 * kept 非空时前端不得移除对应卡片，否则刷新后原样回来。
 */
