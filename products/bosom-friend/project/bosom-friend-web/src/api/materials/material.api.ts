import type { BatchDeleteMediaResult, ConfirmUploadData, CreateMaterialGroupParams, CreateMaterialGroupVo, CreateMaterialParams, GetMaterialGroupBySceneParams, MaterialFilterDeleteParams, MaterialGroupBySceneData, MaterialGroupListFilters, MaterialGroupListVo, MaterialGroupSceneVo, MaterialListFilters, MaterialListQueryParams, MaterialListVo, MaterialOpenApiResponse, MediaListFilters, MediaListResponse, OptimalMaterialVo, PromotionMaterial, ThumbnailVo, TransferMaterialParams, TransferMediaParams, TransferMediaResult, UpdateMaterialGroupParams, UpdateMaterialParams, UploadSignData, UploadToOssOptions } from './material.types'
import type { PlatType } from '@web/app/config/platConfig'
import md5 from 'blueimp-md5'
import { useUserStore } from '@web/store/user'
import { optimizeImageForUpload } from '@web/utils/media'
import http, { request } from '@web/utils/request'
...
export function getVideoThumbnail(url: string, timeInSeconds = 1) {
  // 静默请求：缩略图提取失败时由调用方优雅降级（占位图），不弹全局错误通知
  return http.get<ThumbnailVo>('assets/thumbnail', { url, timeInSeconds }, true)
}
...
/**
 * 批量删除媒体资源
 * 根据ID列表批量删除媒体资源；返回实际删除与被保留（仍被生成记录引用）的 ID。
 */
export function batchDeleteMedia(ids: string[]) {
  return http.delete<BatchDeleteMediaResult>('contents', { ids, kind: 'asset' })
}
...
export function transferMedia(data: TransferMediaParams) {
  return http.post<TransferMediaResult>('contents/transfer', { ...data, kind: 'asset' })
}
...
export function getMediaList(
  ...
  return http.get<MediaListResponse>(`contents/assets/${pageNo}/${pageSize}`, {
    ...filter,
    ...(type ? { type } : {}),
  })
}
...
export function apiCreateMaterialGroup(data: CreateMaterialGroupParams) {
  return http.post<CreateMaterialGroupVo>('contents/groups', {
    ...data,
    type: 'video',
  })
}
...
export function apiDeleteMaterialGroup(id: string) {
  return http.delete(`contents/groups/${id}`)
}
...
export function apiUpdateMaterialGroupInfo(
  ...
  return http.post(`contents/groups/info/${id}`, data)
}
...
export function apiGetMaterialGroupList(pageNo: number, pageSize: number, filters?: MaterialGroupListFilters) {
  return http.get<MaterialGroupListVo>(`contents/groups/list/${pageNo}/${pageSize}`, filters)
}
...
export function apiGetMaterialInfo(id: string) {
  return http.get(`contents/groups/info/${id}`)
}
...
export function apiCreateMaterial(
  ...
  return http.post('contents/drafts', data, silent)
}
...
export function apiBatchDeleteMaterials(ids: string[]) {
  return http.delete('contents', { ids, kind: 'draft' })
}
...
export function apiTransferMaterials(data: TransferMaterialParams) {
  return http.post<TransferMediaResult>('contents/transfer', { ...data, kind: 'draft' })
}
...
export function apiFilterDeleteMaterials(data: MaterialFilterDeleteParams) {
  return http.delete('contents/drafts/filter', data)
}
...
export async function apiGetMaterialList(groupId: string, pageNo: number, pageSize: number, filters?: MaterialListFilters) {
  ...
  const res = await http.get<MaterialListVo>(`contents/drafts/${pageNo}/${pageSize}`, params)

  const list = res?.data?.list
  // 兼容代码，图文草稿补封面。
  // 注意：mediaList 可能为空数组（如纯文案/历史草稿），直接取 [0].type 会抛 TypeError，
  // 导致整个素材列表接口失败、草稿箱显示为空、一键发布无素材可用。
...
export function apiGetDraftInfo(id: string) {
  return http.get<PromotionMaterial>(`contents/${id}`)
}
...
export function apiGetOptimalMaterial(groupId: string, accountType: PlatType) {
  return http.get<OptimalMaterialVo>('/contents/drafts/optimal', { groupId, accountType })
}
...
export async function apiGetMaterialGroupByScene(
  ...
  const res = await http.get<MaterialGroupBySceneData>('/contents/groups/by-scene', params, silent)
  if (!res)
    return res
