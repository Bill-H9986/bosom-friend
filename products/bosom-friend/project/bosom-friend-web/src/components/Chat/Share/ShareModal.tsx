...
import type { TaskDetail } from '@web/api/ai/ai.types'
...
import { convertMessages } from '@web/components/Chat/utils'
...
/**
 * 分享链接必须指向应用自身的路由：桌面端跑在 `/bosom-friend/` 下且使用 hash 路由，
 * 用 origin + `/chat?token=` 拼出来的链接既丢了应用基路径也丢了 `#` 前缀，打开就是 404。
 * @param token - 分享令牌。
 * @returns 可直接打开的只读分享页地址。
 */
function shareUrlForToken(token: string): string {
  if (typeof window === 'undefined')
    return `${APP_SITE_URL}/#/chat?token=${token}`
  const { origin, pathname } = window.location
  const base = pathname.replace(/index\.html?$/i, '')
  return `${origin}${base}#/chat?token=${token}`
}
...
export function ShareModal({ taskId, open = false, onOpenChange, trigger }: ShareModalProps) {
  ...
  const handleGenerateLink = useCallback(async () => {
    ...
    try {
      ...
      if (res?.data) {
        const fullUrl = shareUrlForToken(res.data.token)
  ...
  const handleAgentShare = useCallback(async () => {
    ...
    if (!link) {
      ...
      try {
        const res = await agentApi.createPublicShare(taskId, validitySeconds)
        if (res?.data) {
          link = shareUrlForToken(res.data.token)
          setShareLink(link)
          setShareExpiresAt(res.data.expiresAt)
        }
      }
  ...
  const handleDownloadImage = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < blobs.length; i++) {
        ...
        a.download
          = blobs.length === 1
            ? `zhiyin_conversation_${taskId}.png`
            : `zhiyin_${taskId}_${i + 1}.png`
