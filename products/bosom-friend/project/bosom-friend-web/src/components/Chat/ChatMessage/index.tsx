...
import type { IActionCard, IMessageStep, IPublishFlowData, IWorkflowStep } from '@web/store/agent'
import { AlertCircle, Loader2, Pencil, Play } from 'lucide-react'
import { useCallback, useMemo, useState } from 'react'
import ReactMarkdown from 'react-markdown'
import rehypeRaw from 'rehype-raw'
import remarkGfm from 'remark-gfm'
import { useTransClient } from '@web/app/i18n/client'
import MediaGallery from '@web/components/Chat/ChatMessage/MediaGallery'
import WorkflowSection from '@web/components/Chat/ChatMessage/WorkflowComponents'
...
export interface IChatMessageProps {
  ...
  /** 自定义类名 */
  className?: string
  /** 消息 ID（对话可修改：用户消息内联编辑使用） */
  id?: string
  /** 用户消息是否被编辑过 */
  edited?: boolean
  /** 编辑消息回调（保存时触发，由列表层调用 store 的 editUserMessage） */
  ...
  return (
    <div className={cn('flex', isUser ? 'justify-end' : 'justify-start', className)}>
      ...
      <div className={cn('flex flex-col gap-2 min-w-0', isUser ? 'items-end' : 'items-start')}>
            ...
            {isGenerating && (
              <div className="mt-4 pt-4 border-t border-border/50">
                <div className="flex items-center gap-3 px-1">
                  ...
                  {/* 思考文字 */}
                  <span
                    className="text-sm font-medium bg-linear-to-r from-pink-500 via-purple-500 to-violet-500 bg-clip-text text-transparent"
                    style={{
        ...
        {/* 用户消息：可编辑气泡（hover 操作栏 + 内联编辑，观感提升） */}
        {isUser && content && (
          <div className="group relative max-w-[85%]">
            {editing ? (
              <div className="flex flex-col gap-2 rounded-2xl rounded-br-md bg-muted p-3 ring-1 ring-primary/40 shadow-lg">
                <textarea
                  autoFocus
                  value={editDraft}
                ...
                <div className="flex items-center justify-end gap-2">
                  <button
                    ...
                    className="cursor-pointer rounded-lg px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-background"
                  >
                    {t('message.cancel')}
                  </button>
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (next && next !== filterSystemPromoInfo(content)) {
                        onEditMessage?.(id ?? '', next)
                      }
                      setEditing(false)
                      setEditDraft('')
                    }}
                    className="cursor-pointer rounded-lg bg-primary px-3 py-1 text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/85"
                  >
                    {t('message.save')}
                  </button>
                </div>
              ...
              <div
                className={cn(
                  ...
                  'group-hover:bg-muted/80 group-hover:shadow-sm',
                )}
            ...
            {/* hover 操作栏：编辑 */}
            {!editing && onEditMessage && id && (
              <div className="absolute -top-2.5 right-2 flex items-center gap-1 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                <button
                  type="button"
                  onClick={() => {
                    setEditDraft(filterSystemPromoInfo(content))
                    setEditing(true)
                  }}
                  className="flex cursor-pointer items-center gap-1 rounded-full border border-border/60 bg-background px-2 py-0.5 text-[11px] text-muted-foreground shadow-sm transition-colors hover:border-primary/40 hover:text-primary"
                >
                  <Pencil className="h-3 w-3" />
                  {t('message.edit')}
                </button>
                {edited && (
                  <span className="rounded-full bg-background/90 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                    {t('message.edited')}
                  </span>
                )}
              </div>
