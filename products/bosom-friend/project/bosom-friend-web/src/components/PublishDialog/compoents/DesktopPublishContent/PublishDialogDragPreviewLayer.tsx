...
import { PUBLISH_DIALOG_DND_TYPE } from '@web/components/PublishDialog/PublishDialog.util'
...
function getPreviewData(item: PublishDialogDragItem) {
  if (item.kind === 'draft') {
    ...
    return {
      ...
      src: item.material.coverUrl || imageMedia?.url || resolveAsset('images/placeholder.png'),
  ...
  return {
    ...
    src: item.media.thumbUrl || item.media.url || resolveAsset('images/placeholder.png'),
