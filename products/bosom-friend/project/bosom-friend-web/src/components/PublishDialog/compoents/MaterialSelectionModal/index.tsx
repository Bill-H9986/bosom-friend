...
import { getAgentAssets } from '@web/api/ai/ai.api'
import { apiGetMaterialGroupList, getMediaList } from '@web/api/materials/material.api'
...
import { convertAssetToMediaItem, filterAssetsByMediaType } from '@web/utils/agent/asset'
...
const MaterialSelectionModalContent = memo(
  ({ onOpenChange, mediaTypes, onSelect }: Omit<MaterialSelectionModalProps, 'open'>) => {
    ...
    const fetchMediaList = useCallback(async (materialGroupId: string) => {
      ...
      try {
        const response = await getMediaList({ groupId: materialGroupId }, 1, PAGE_SIZE)
        if (response?.data?.list) {
    ...
    const loadMoreMedia = useCallback(async () => {
      ...
      try {
        const response = await getMediaList({ groupId: selectedGroup._id }, nextPage, PAGE_SIZE)
        if (response?.data?.list) {
