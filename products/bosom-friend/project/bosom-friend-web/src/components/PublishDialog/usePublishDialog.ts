...
import type { ErrPubParamsMapType } from '@web/components/PublishDialog/hooks/usePubParamsVerify'
import type { IPlatOption, IPubParams, PubItem } from '@web/components/PublishDialog/publishDialog.type'
...
import { getSocialAccountIdentityKeys, isPublishTitleSupported, isSameSocialAccount } from '@web/components/PublishDialog/PublishDialog.util'
import { usePublishDialogStorageStore } from '@web/components/PublishDialog/usePublishDialogStorageStore'
import { getPlatformInfoSync, isPlatformEnabledSync } from '@web/store/platformMetadata'
      ...
      const methods = {
        ...
        setAccountAllParams(pubParmas: Partial<IPubParams>) {
          ...
          for (let i = 0; i < pubList.length; i++) {
            ...
            if (
              Object.hasOwn(pubParmas, 'video')
              && (platConfig === undefined || platConfig.pubTypes.has(PubType.VIDEO))
            ) {
            ...
            if (
              Object.hasOwn(pubParmas, 'images')
              && (platConfig === undefined || platConfig.pubTypes.has(PubType.ImageText))
            ) {
