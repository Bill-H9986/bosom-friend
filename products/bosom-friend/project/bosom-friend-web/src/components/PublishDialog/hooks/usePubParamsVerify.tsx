import type { PlatformInfo, PlatformMediaRules } from '@web/api/channels/channel.types'
import type { IImgFile, IVideoFile, PubItem } from '@web/components/PublishDialog/publishDialog.type'
...
import { UploadTaskStatusEnum } from '@web/components/PublishDialog/compoents/PublishManageUpload/publishManageUpload.enum'
...
import { getFilePathName, parseTopicString } from '@web/utils/common'
import { formatFileSize, getDurationPartsFromSeconds } from '@web/utils/format'
...
export default function usePubParamsVerify(data: PubItem[]) {
  ...
  const errParamsMap = useMemo(() => {
    ...
    for (const v of data) {
      ...
      // 小红书/抖音等真实发布通道必须有媒体和标题，否则接口只会生成失败记录。
      if ((v.account.type === PlatType.Xhs || v.account.type === PlatType.Douyin) && !hasImages && !hasVideo) {
        addErrorMsg(t('validation.uploadImageOrVideo'))
      }
      if ((v.account.type === PlatType.Xhs || v.account.type === PlatType.Douyin) && !v.params.title) {
