...
import type { VideoModelInfo } from '@web/api/ai/ai.types'
import { useMemo } from 'react'
import {
  enabledVideoDurationOptions,
  ...
  const selectedVideoModelParamInfos = useMemo<VideoModelParamInfo[]>(() => {
    return selectedVideoModelInfos.map((model) => {
      ...
      return {
        ...
        aspectRatios,
        durationLimits,
        // 本板块的档位先收一遍（超长短视频上限的不出现），UI 与校验用同一份口径。
        durationOptions: enabledVideoDurationOptions(model.durations),
      }
    })
