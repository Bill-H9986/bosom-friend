...
import { apiCreateDraftFromVideoUrl } from '@web/api/ai/ai.api'
import { apiBatchDeleteMaterials, apiGetMaterialList, batchDeleteMedia, getMediaList } from '@web/api/materials/material.api'
...
} from '@web/store/draft-box/materialSync'
...
/** 批量删除结果：kept 为服务端保留（仍被生成记录引用）的素材数，前端按它决定是否提示未删除。 */
  ...
  combine(
    ...
    (set, get) => ({
      ...
      fetchMediaList: async (materialGroupId: string, type: 'video' | 'img') => {
        ...
        try {
          const res = await getMediaList({ groupId: materialGroupId }, 1, PAGE_SIZE, type)
          if (get().activeMaterialGroupId !== materialGroupId) {
      ...
      loadMore: async (materialGroupId: string, type: 'video' | 'img') => {
        ...
        try {
          const res = await getMediaList({ groupId: materialGroupId }, nextPage, PAGE_SIZE, type)
          if (res?.data) {
      ...
      fetchAllList: async (materialGroupId: string, planId: string) => {
        ...
        try {
          const [draftRes, videoRes, imgRes] = await Promise.all([
            ...
            getMediaList({ groupId: materialGroupId }, 1, ALL_PAGE_SIZE, 'video'),
            getMediaList({ groupId: materialGroupId }, 1, ALL_PAGE_SIZE, 'img'),
          ])
      ...
      loadMoreAll: async (materialGroupId: string, planId: string) => {
        ...
        try {
          ...
          if (all.videoHasMore) {
            fetches.push(getMediaList({ groupId: materialGroupId }, all.videoPage + 1, ALL_PAGE_SIZE, 'video'))
            fetchTypes.push('video')
          }
          if (all.imgHasMore) {
            fetches.push(getMediaList({ groupId: materialGroupId }, all.imgPage + 1, ALL_PAGE_SIZE, 'img'))
            fetchTypes.push('img')
          }
      ...
      /**
       * 批量删除媒体（视频/图片 Tab）
       * 只移除服务端确认删除的项：被生成记录引用的素材服务端会保留，前端乐观移除会让卡片刷新后回魂。
       */
      batchDeleteByType: async (materialGroupId: string, type: 'video' | 'img') => {
        ...
        try {
          ...
          methods.removeItemsFromAll(deletedIds, type)

          return { ok: true, deleted: deletedIds.length, kept: keptIds.length }
        }
        catch {
          return { ok: false, deleted: 0, kept: 0 }
        }
       ...
       * 草稿删除是级联删除，素材删除按服务端实际结果更新（被生成记录引用的会保留）。
       */
      batchDeleteAll: async (materialGroupId: string, planId: string) => {
        ...
        try {
          const [draftRes, mediaRes] = await Promise.all([
            draftIds.length > 0 ? apiBatchDeleteMaterials(draftIds) : Promise.resolve(null),
            mediaIds.length > 0 ? batchDeleteMedia(mediaIds) : Promise.resolve(null),
          ])

          if (draftRes !== null && draftRes.code !== 0)
            return { ok: false, deleted: 0, kept: 0 }
          if (mediaRes !== null && mediaRes.code !== 0)
            return { ok: false, deleted: 0, kept: 0 }

          const deletedMediaIds = mediaRes?.data?.deleted ?? []
          const keptMediaIds = mediaRes?.data?.kept ?? []
          const deletedMediaSet = new Set(deletedMediaIds)

          // 从 all.mergedList 中移除真正删除的项
          const deletedSet = new Set([...draftIds, ...deletedMediaIds])
          const current = get().all
          const newMergedList = current.mergedList.filter(item => !deletedSet.has(item.id))

          const videoDeletedIds = entries.filter(([id, s]) => s === 'video' && deletedMediaSet.has(id)).map(([id]) => id)
          const imgDeletedIds = entries.filter(([id, s]) => s === 'img' && deletedMediaSet.has(id)).map(([id]) => id)

          set({
            all: {
              ...current,
              mergedList: newMergedList,
              draftTotal: Math.max(0, current.draftTotal - draftIds.length),
              videoTotal: Math.max(0, current.videoTotal - videoDeletedIds.length),
              imgTotal: Math.max(0, current.imgTotal - imgDeletedIds.length),
            },
            batchMode: false,
            selectedItems: {},
          })
          ...
          return { ok: true, deleted: draftIds.length + deletedMediaIds.length, kept: keptMediaIds.length }
        }
        catch {
          return { ok: false, deleted: 0, kept: 0 }
        }
