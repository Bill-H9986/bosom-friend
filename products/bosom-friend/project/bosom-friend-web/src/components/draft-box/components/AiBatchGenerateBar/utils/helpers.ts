import type { ImageModelInfo, VideoModelInfo, VideoModelInputConstraint } from '@web/api/ai/ai.types'
import type { IUploadedMedia } from '@web/components/Chat/MediaUpload'
import type { IPersistedMedia, VideoModelParams } from '@web/store/draft-box/draftBoxConfigStore'
import {
...
export function buildAggregateImagePricing(models: ImageModelInfo[]) {
  // 按“分辨率 + 比例”成对聚合：同一档位在不同比例下都是独立选项，
  // 避免 find(按分辨率) 只取到第一项导致 1080p(9:16) 被错误过滤。
  const pairMap = new Map<string, { resolution: string; aspectRatio?: string; size?: string }>()
  for (const model of models) {
    for (const item of model.pricing) {
      const ratio = item.aspectRatio ?? ''
      const key = item.resolution + '|' + ratio
      if (!pairMap.has(key)) {
        pairMap.set(key, { resolution: item.resolution, aspectRatio: item.aspectRatio, size: item.size })
      }
    }
  }
  return [...pairMap.values()].map(({ resolution, aspectRatio, size }) => ({
    resolution,
    aspectRatio,
    size,
    pricePerImage:
      Math.ceil(
        models.reduce((sum, model) => {
          ...
          return (
            sum + (model.pricing.find(item => item.resolution === resolution && (item.aspectRatio ?? '') === ratio)?.pricePerImage ?? 0)
          )
        }, 0) * 100,
      ) / 100,
  }))
