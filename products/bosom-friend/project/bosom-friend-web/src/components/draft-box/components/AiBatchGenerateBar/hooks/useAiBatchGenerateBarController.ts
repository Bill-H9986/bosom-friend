...
import type { DraftContentType, ImageModelInfo, VideoModelInfo, VideoModelType } from '@web/api/ai/ai.types'
...
import { useTaskPlatforms } from '@web/hooks/usePlatformMetadata'
...
export function useAiBatchGenerateBarController({
  ...
  useEffect(() => {
    ...
    const ratioPricing = contentType === 'image_text'
      ? nextPricing.filter(p => !p.aspectRatio || p.aspectRatio === aspectRatio)
      : nextPricing
    if (
      ratioPricing.length > 0
      && !includesOption(
        ratioPricing.map(p => p.resolution),
        imageSize,
      )
    ) {
      const firstSize = ratioPricing[0]?.resolution ?? '720p'
  ...
  useEffect(() => {
    ...
    if ((configSnapshot.style ?? '') !== style) {
      setStyle(configSnapshot.style ?? '')
    }
  ...
  const handleImageModelsChange = useCallback(
    (models: string[]) => {
      ...
      if (
        ...
        const firstSize = commonPricing[0]?.resolution ?? '720p'
        setImageSize(firstSize)
        updateConfig(configKey, { imageSize: firstSize })
      }
  ...
  const handleDurationChange = useCallback(
    (d: number) => {
      const safeDuration = sanitizeVideoDuration(d)
      ...
      if (modelType) {
        ...
        const nextVideoModelParams = {
          ...videoModelParams,
          [modelType]: { ...currentParams, duration: safeDuration },
        }
        setVideoModelParams(nextVideoModelParams)
        updateConfig(configKey, { duration: safeDuration, videoModelParams: nextVideoModelParams })
        return
      }
      updateConfig(configKey, { duration: safeDuration })
    },
    [configKey, modelType, resolvedVideoModelParams, updateConfig, videoModelParams],
  )
    ...
    (model: VideoModelType, params: VideoModelParams) => {
      ...
      if (videoModelSelectionMode === 'single' && model === modelType) {
        ...
        if (params.duration !== undefined) {
          const safeDuration = sanitizeVideoDuration(params.duration)
          setDuration(safeDuration)
          updateConfig(configKey, { duration: safeDuration })
        }
  ...
  const handleVideoModelsChange = useCallback(
    (models: VideoModelType[]) => {
      ...
      const nextPrimaryDuration = sanitizeVideoDuration(nextPrimaryParams?.duration ?? duration)
  ...
  const handleStyleChange = useCallback(
    (value: string) => {
      setStyle(value)
      updateConfig(configKey, { style: value })
    },
    [configKey, setStyle, updateConfig],
  )
  ...
  const handleReset = useCallback(() => {
    ...
    const nextConfig = {
      ...
      imageCount: 3,
      imageSize: firstImageModel?.pricing?.[0]?.resolution ?? '720p',
