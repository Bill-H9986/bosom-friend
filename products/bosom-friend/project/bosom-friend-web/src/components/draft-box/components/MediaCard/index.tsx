...
export const MediaCard = memo(({ media, onClick, batchMode, selected, onToggleSelect, actions, useOssThumbnail, enablePublishDrag }: MediaCardProps) => {
  ...
  const thumbUrl = media.thumbUrl ? getOssUrl(media.thumbUrl) : media.url ? getOssUrl(media.url) : resolveAsset('images/placeholder.png')
  ...
  const cardNode = (
    <div
      className={cn(
        'mb-4 cursor-pointer group relative transition-all duration-300 ease-apple-out hover:-translate-y-0.5 hover:shadow-xl hover:shadow-primary/10',
