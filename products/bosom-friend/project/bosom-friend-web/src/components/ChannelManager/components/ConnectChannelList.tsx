...
import { useRegionSortedPlatforms } from '@web/hooks/usePlatformMetadata'
import { useAccountStore } from '@web/store/account'
import { isChannelPlatform, isPlatformComingSoon, isPlatformRegionLimited } from '@web/store/platformMetadata/utils'
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <ScrollArea className="flex-1 p-4">
        ...
        >
          <TooltipProvider>
            {platforms.filter(([, value]) => isChannelPlatform(value)).map(([key, value]) => {
                  ...
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={`
                        ...
                        p-2 transition-all duration-300 ease-apple-out
                        hover:-translate-y-0.5 hover:border-primary/25 hover:bg-accent
                    ...
                    >
                      ...
                      {/* 浏览器插件登录状态标签（官方插件式：已登录/去登录） */}
                      {isBrowserPlugin && (
                        <span
                          className={`absolute -right-px -top-px z-20 rounded-bl rounded-tr-lg px-1.5 py-0.5 text-[10px] font-medium ${
                            accountList.some(account => account.type === key)
                              ? 'bg-success/15 text-success'
                              : 'bg-muted text-muted-foreground'
                          }`}
                        >
                          {accountList.some(account => account.type === key)
                            ? t('channelManager.loggedIn')
                            : t('channelManager.goLogin')}
                        </span>
                      )}
                      {isQrAuth && (
