...
import { usePlatformInfoMap, useRegionSortedPlatforms } from '@web/hooks/usePlatformMetadata'
...
export function ChannelSidebar() {
  ...
  // 获取所有平台列表（只保留产品真实支持的频道平台，不再展示一大排未接通平台）
  const allPlatforms = useMemo<PlatType[]>(() => {
    return regionSortedPlatforms
      .filter(([, info]) => isChannelPlatform(info))
