...
import { logoutAccountApi } from '@web/api/accounts/account.api'
import { Loader2, LogOut, RefreshCw, Trash2 } from 'lucide-react'

import { useState } from 'react'
import AccountStatusView from '@web/app/[lng]/accounts/components/AccountsTopNav/components/AccountStatusView'
...
import { shouldShowFansSyncNotice } from '@web/utils/social/account'
...
export function ChannelItem({
  ...
  // 处理退出登录：删除账号并清理该平台浏览器会话（官方插件式退出）
  const handleLogout = async () => {
    const confirmed = await confirm({
      title: t('channelManager.logoutAccountConfirmTitle'),
      content: t('channelManager.logoutAccountConfirmContent', { name: channel.nickname || channel.type }),
      okText: t('channelManager.logoutAccount'),
    })
    if (!confirmed) {
      return
    }
    ...
    try {
      const ipc = window.ipcRenderer as unknown as { __zyShim?: boolean }
      if (window.ipcRenderer && ipc.__zyShim !== true) {
        await window.ipcRenderer.invoke('ICP_PLATFORM_ACCOUNT_LOGOUT', Number(channel.id), channel.type)
      }
      else {
        await logoutAccountApi(channel.id)
      }
      await useAccountStore.getState().getAccountList(false, { force: true })
      await useAccountStore.getState().getAccountGroup()
      toast.success(t('channelManager.logoutAccountSuccess'))
    }
    catch {
      toast.error(t('channelManager.logoutAccountFailed'))
    }
    finally {
    ...
    >
      ...
      {isDeleting ? (
        ...
        <>
          {/* 退出登录：删除账号并清除平台会话 */}
          <Button
            data-testid="cm-channel-logout-btn"
            variant="ghost"
            size="sm"
            disabled={logoutLoading}
            onClick={handleLogout}
            className="h-8 w-8 shrink-0 rounded-lg p-0 text-muted-foreground hover:bg-warning/10 hover:text-warning"
          >
            {logoutLoading ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
