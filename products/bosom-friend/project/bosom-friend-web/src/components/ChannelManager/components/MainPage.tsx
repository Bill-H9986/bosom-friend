...
import { apiUpdateAccountGroupSortRank, deleteAccountApi, deleteAccountGroupApi, updateAccountGroupApi } from '@web/api/accounts/account.api'
...
import { deleteDesktopAccount, deleteDesktopAccountGroup } from '../utils/desktopLogin'
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      if (deleteDialog.type === 'space') {
        const deleted = await deleteDesktopAccountGroup(deleteDialog.id)
        if (!deleted) {
        ...
        const deleted = await deleteDesktopAccount(deleteDialog.id)
        if (!deleted) {
