...
import { APP_DOWNLOAD_DOCS_URL, APP_RELEASE_API as APP_RELEASE_API_URL } from '@web/config/app'
...
// 主应用下载配置（默认英文）
export const MAIN_APP_DOWNLOAD_URL = APP_DOWNLOAD_DOCS_URL

// API端点
const APP_RELEASE_API = APP_RELEASE_API_URL
...
export async function fetchLatestDownloadUrl(): Promise<string | null> {
  try {
    ...
    inflightRequest = (async () => {
      ...
      if (!response.ok) {
        console.error('应用版本信息获取失败：', response.statusText)
        return null
      }
    ...
    console.error('最新下载地址获取失败：', error)
    inflightRequest = null
    return null
  }
}
export async function getMainAppDownloadUrl(lng?: string): Promise<string> {
  ...
  return isZh ? APP_DOWNLOAD_DOCS_URL : APP_DOWNLOAD_DOCS_URL
}
...
export const APP_DOWNLOAD_CONFIGS: Record<string, AppDownloadConfig> = {
  xhs: {
    ...
    description:
      '小红书相关操作需要在手机 App 中完成。请下载并安装小红书 App 后继续。',
    tips: [
      '安装后请在 App 中登录您的账号',
      '请确保您的账号已完成实名认证',
      '在 App 中完成相关任务操作',
  ...
  douyin: {
    ...
    qrCodeUrl: '',
    description:
      '抖音相关操作需要在手机 App 中完成。请下载并安装抖音 App 后继续。',
    tips: [
      '安装后请在 App 中登录您的账号',
      '请确保您的账号已完成实名认证',
      '在 App 中完成相关任务操作',
