...
import { Cpu, Globe, UploadCloud } from 'lucide-react'
...
import { CustomLlmTab } from './tabs/CustomLlmTab'
import { OtaTelemetryTab } from './tabs/OtaTelemetryTab'
...
/** 设置页面类型（导出供外部使用） */
export type { SettingsTab } from '@web/store/settingsModal'
...
export function SettingsModal({ open, onClose, defaultTab }: SettingsModalProps) {
  ...
  // 首开默认「通用」：tabConfigs 无 profile 页签，初始 profile 会命中 default 分支导致内容区空白
  ...
  const tabConfigs: TabConfig[] = [
    ...
    { key: 'customLlm', icon: <Cpu className="h-4 w-4" />, label: '自定义大模型', requireAuth: true },
    { key: 'ota', icon: <UploadCloud className="h-4 w-4" />, label: '系统与更新' },
  ]

  // 根据登录状态和 placeId 过滤 Tab
  const visibleTabs = tabConfigs.filter((tab) => {
    // 免登录模式（登录页暂时屏蔽期间）：自定义大模型不再要求登录态，
    // 保证无 token 时也能配置模型 Key（BYOK 是 AI 功能的唯一入口）。
    if (tab.requireAuth && !isLoggedIn && LOGIN_ENABLED)
      return false
    return true
  })
              ...
              <div className="flex flex-col gap-8">
                <section className="space-y-4">
                  <div className="border-b border-border pb-2.5">
                    <h3 className="text-sm font-semibold text-foreground">{t('tabs.profile')}</h3>
                  </div>
                  <ProfileTab onClose={onClose} />
                </section>
                <section className="space-y-4">
                  <div className="border-b border-border pb-2.5">
                    <h3 className="text-sm font-semibold text-foreground">{t('tabs.general')}</h3>
                  </div>
                  <GeneralTab />
                </section>
              </div>
        ...
        <div className="flex h-[85vh] max-h-[650px] flex-col overflow-hidden md:h-[70vh] md:max-h-none md:flex-row">
          ...
          <div className="flex w-full shrink-0 overflow-hidden flex-col border-b border-border md:w-52 md:border-b-0 md:border-r">
            ...
            <div className="flex h-14 shrink-0 items-center gap-2 px-4 md:h-auto md:px-5 md:py-4">
              <Image src={logo} alt="Bosom Friend" width={24} height={24} className="hidden md:block md:h-7 md:w-7" />
              <span className="text-sm font-semibold tracking-tight text-foreground md:text-base">
                <span className="md:hidden">{t('title')}</span>
                <span className="hidden md:inline">Bosom Friend</span>
              </span>
            </div>
              ...
              {visibleTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'btn btn-ghost btn-md w-full cursor-pointer justify-start rounded-lg transition-colors duration-200',
                      isActive
                        ? 'bg-brand-cyan/10 font-medium text-brand-purple-deep'
