...
export interface UseNewWorkOptions {
  /**
   * 打开发布弹窗。宿主由布局壳统一挂载，调用方只负责把参数（默认账号、预设发布时间）交出去。
   *
   * @param options.defaultAccountIds - 默认选中的账号 id；undefined 表示交给弹窗自己选。
   * @param options.date - 预设发布时间（ISO 字符串）。
   */
  openPublishDialog: (options: { defaultAccountIds?: string[]; date?: string }) => void
}
 ...
 * @example
 * ```tsx
 * const { openNewWork, hasUsableAccounts } = useNewWork({ openPublishDialog })
 *
 * // 基础用法：打开新建作品弹窗
...
export function useNewWork(options: UseNewWorkOptions) {
  const { openPublishDialog } = options
  ...
  const openNewWork = useCallback(
    (workOptions?: OpenNewWorkOptions) => {
      ...
      // 一次调用把「默认账号 + 预设发布时间」都交给宿主，宿主是唯一打开入口。
      openPublishDialog({ defaultAccountIds: getDefaultAccountIds(), date })
    },
    [getDefaultAccountIds, openPublishDialog],
  )
