...
import type { CalendarViewType } from '@web/store/system'
import dayGridPlugin from '@fullcalendar/daygrid'
import FullCalendar from '@fullcalendar/react'
import dayjs from 'dayjs'
import { useSearchParams } from '@web/next-shims/navigation'
...
import CalendarToolbar from '@web/app/[lng]/accounts/components/CalendarTiming/CalendarToolbar'
import { useCalendarTiming } from '@web/app/[lng]/accounts/components/CalendarTiming/useCalendarTiming'
import { useNewWork } from '@web/app/[lng]/accounts/hooks/useNewWork'
import { openPublishDialog, publishDialogHandle } from '@web/store/publishDialogHost'
...
const CalendarTiming = memo(
  forwardRef(({}: ICalendarTimingProps, ref: ForwardedRef<ICalendarTimingRef>) => {
    ...
    const calendarQueryRef = useRef({ currentDate, calendarViewType: useSystemStore.getState().calendarViewType })
    ...
    calendarQueryRef.current = { currentDate, calendarViewType }
    ...
    /**
     * 打开发布弹窗：宿主挂在布局壳，本页只把参数递过去；带日期时先给宿主句柄设发布时间。
     *
     * @param options.defaultAccountIds - 默认选中的账号。
     * @param options.date - 预设发布时间（ISO 字符串）。
     */
    const openPublish = useCallback((options: { defaultAccountIds?: string[]; date?: string }) => {
      setDefaultAccountIds(options.defaultAccountIds)
      if (options.date !== undefined)
        publishDialogHandle()?.setPubTime(options.date)
      openPublishDialog({
        accounts: accountList,
        accountListInitialLoading,
        defaultAccountIds: options.defaultAccountIds,
        onPubSuccess: () => {
          getPubRecord({
        ...
        onClosed: () => setDefaultAccountIds(undefined),
      })
    }, [accountList, accountListInitialLoading, getPubRecord, calendarViewType, currentDate])

    // 使用新建作品 hook
    const { openNewWork } = useNewWork({ openPublishDialog: openPublish })
    ...
    // 监听 URL 参数，自动打开发布弹窗
    useEffect(() => {
      // 不能叫 openPublish：同作用域已有同名的 useCallback 函数，
      // 局部 const 会把它遮蔽，openPublish({}) 就变成调用一个字符串。
      const openPublishParam = searchParams.get('openPublish')
      const fromSignIn = searchParams.get('fromSignIn')

      if (openPublishParam === 'true' && fromSignIn === 'true') {
        openPublish({})
    ...
    // 用户从其他页面/窗口回到发布日历时自动刷新，避免看到发布前加载的旧记录
    useEffect(() => {
      if (isMobile || window.matchMedia('(max-width: 767px)').matches) {
        return
      }
      ...
      const refreshIfVisible = () => {
        ...
        getPubRecord({
          dateRange: query.calendarViewType === 'week'
            ? getWeekDateRange(query.currentDate)
            : getMonthDateRange(query.currentDate),
        })
      }
