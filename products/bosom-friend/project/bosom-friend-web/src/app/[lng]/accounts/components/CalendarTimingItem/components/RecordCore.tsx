...
import type { ChannelPublishUserActionVo, ChannelWorkMetricsSnapshot } from '@web/api/channels/channel.types'
import type { PublishRecordEngagement, PublishRecordItem } from '@web/api/platforms/publish.types'
import type { PlatformPublishTask } from '@web/store/plugin/types/baseTypes'
...
} from '@web/api/channels/channel.api'
import { PublishStatus } from '@web/api/platforms/publish.constants'
import { ClientType } from '@web/app/[lng]/accounts/accounts.enums'
import {
...
import { useAccountsWorkAnalyticsCacheStore } from '@web/app/[lng]/accounts/workAnalyticsCacheStore'
import { PlatType } from '@web/app/config/platConfig'
import { useTransClient } from '@web/app/i18n/client'
import AvatarPlat from '@web/components/common/AvatarPlat'
import { MediaPreview } from '@web/components/common/MediaPreview'
import { OssImage } from '@web/components/common/OssImage'
import { PublishDetailModal } from '@web/components/Plugin/PublishDetailModal'
import PublishDatePicker from '@web/components/PublishDialog/compoents/PublishDatePicker'
...
import { usePluginStore } from '@web/store/plugin'
import { PlatformTaskStatus } from '@web/store/plugin/types/baseTypes'
...
function hasEngagementData(engagement?: PublishRecordEngagement) {
  return Boolean(
    engagement
    && Object.values(engagement).some(value => typeof value === 'number' && Number.isFinite(value) && value > 0),
  )
}
...
function normalizeWorkMetrics(metrics?: ChannelWorkMetricsSnapshot): PublishRecordEngagement | undefined {
  ...
  const pick = (...candidates: Array<number | undefined>) =>
    candidates.find(value => typeof value === 'number' && Number.isFinite(value))
  return {
    viewCount: pick(metrics.viewCount, metrics.playCount),
    commentCount: pick(metrics.commentCount),
    likeCount: pick(metrics.likeCount),
    shareCount: pick(metrics.shareCount),
    clickCount: pick(metrics.clickCount),
    impressionCount: pick(metrics.impressionCount),
    favoriteCount: pick(metrics.collectCount, metrics.saveCount),
  }
}
...
const RecordCore = memo(
  forwardRef(({ publishRecord, dragPreviewWidth }: IRecordCoreProps, ref: ForwardedRef<IRecordCoreRef>) => {
    ...
    const loadWorkMetrics = useCallback(async (record: PublishRecordItem, requestId: number) => {
      if (record.status !== PublishStatus.RELEASED) {
        setWorkEngagement(null)
        setWorkMetricsFetchedAt(null)
        return
      }
      // 记录自带互动数据（桌面端真实采集回填）优先使用，
      // 避免请求平台分析接口后返回空值覆盖真实数据
      if (hasEngagementData(record.engagement)) {
        setWorkEngagement(record.engagement ?? null)
        setWorkMetricsFetchedAt(null)
        setWorkMetricsLoading(false)
        return
      }
      if (!record.platformWorkId || !record.accountId) {
        setWorkEngagement(null)
        setWorkMetricsFetchedAt(null)
        return
      }
    ...
    const TriggerButton = (
      <Button
        ...
        title={activeRecord.title || `${days.format('YYYY-MM-DD HH:mm')} · ${activeRecord.accountType}`}
        aria-label={activeRecord.title || `${days.format('YYYY-MM-DD HH:mm')} · ${activeRecord.accountType}`}
      ...
      >
        <div className={cn('flex items-center', isMobile ? 'gap-2.5' : 'gap-1.5')}>
          ...
          <div className="flex min-w-0 flex-col">
            ...
            {!isMobile && activeRecord.title && (
              <div className="max-w-28 truncate text-[10px] leading-3 text-muted-foreground">
                {activeRecord.title}
              </div>
            )}
          </div>
                    ...
                    <div key={v.label} className="flex-shrink-0 md:flex-1 min-w-[60px] md:min-w-0">
                      ...
                      {activeEngagement && (
                        <div className="text-sm md:text-base font-semibold mt-0.5 md:mt-1">
                          {activeEngagement[v.key as 'viewCount'] ?? t('record.metrics.notCollected')}
                        </div>
                      )}
                    </div>
                  ))}
