    ...
    return (
      <div className={cn('min-w-0 max-w-full overflow-hidden', className)} title={title} aria-label={title}>
        <Popover>
          ...
          <div
            className={cn(
              'max-h-8 min-w-0 max-w-full overflow-hidden px-1 text-left text-[10px] font-medium leading-4 tracking-tight text-brand-purple-deep/80',
              summaryClassName,
            )}
          >
            {displayFestivals.map(({ item, index }, visibleIndex) => {
              ...
              return (
                <span key={tabValue} className="inline">
                  {visibleIndex > 0 && <span className="text-brand-purple-deep/70"> · </span>}
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      className={cn(
