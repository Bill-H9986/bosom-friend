...
import { useRouter, useSearchParams } from '@web/next-shims/navigation'
import { useMemo, useState } from 'react'
import { useForm } from 'react-hook-form'
import { z } from 'zod'

import { emailCodeLoginApi, sendEmailCodeApi } from '@web/api/auth/auth.api'
...
export function EmailLoginForm({
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      if (res?.code === 0) {
        // 本地模式（未配置 SMTP）：后端随响应返回验证码，自动填入并提示
        const devCode = (res?.data as { devCode?: string } | null)?.devCode
        if (devCode) {
          form.setValue('code', devCode)
          toast.success(`本地模式验证码：${devCode}（已自动填入）`)
        }
        else {
