import type { TaskDetail, TaskMessage } from '@web/api/ai/ai.types'
import type { IDisplayMessage, IWorkflowStep } from '@web/store/agent'
import { useCallback, useEffect, useRef, useState } from 'react'
import { useShallow } from 'zustand/react/shallow'
import { agentApi } from '@web/api/ai/ai.api'
import { useAgentStore } from '@web/store/agent'
import { getDefaultTaskData } from '@web/store/agent/agent.state'
...
export function useChatState(options: IChatStateOptions): IChatStateReturn {
  ...
  // 全局 Store 状态 - 获取任务级数据
  const { currentTaskId, taskMessages, setMessages, debugFiles, selectTask } = useAgentStore(
    useShallow(state => ({
  ...
  // 右侧 AI 助手面板跟随当前所选任务：
  // 在任务记录中切换到某个任务时，同步全局 currentTaskId，
  // 使右侧面板展示与所选任务一致的对话（不干扰 SSE 实时生成的临时任务）。
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // store 里已有消息时省掉消息拉取（支持任务缓存与流式追加），但任务详情必须照拉：
    // 只信 store 会让刷新后的详情页标题回落「新对话」，收藏与评分状态丢失
    // （实测：同一 URL 刷新后「取消收藏」变回「收藏」、五星变空星）。
    const hasCachedMessages = storeMessages.length > 0
    ...
    const loadTask = async () => {
      ...
      try {
        ...
        if (result.code === 0 && result.data) {
          ...
          if (result.data.messages && !hasCachedMessages) {
