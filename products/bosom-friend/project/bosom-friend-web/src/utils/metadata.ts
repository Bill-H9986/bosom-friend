...
import { getHreflang, languages } from '@web/app/i18n/languageConfig'
import { getPageTitle } from './title'

const BRAND_KEYWORDS = ['Bosom Friend', 'Bosom Friend', 'AI内容营销', '内容创作发布', '自动客户接待']
...
export async function getMetadata(props: Metadata, lng: string, path?: string): Promise<Metadata> {
  const pagePath = path || '/'
  const baseUrl = process.env.NEXT_PUBLIC_HOST_URL || 'https://bosomfriend.local'
