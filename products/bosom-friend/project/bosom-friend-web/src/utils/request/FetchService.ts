 ...
 * T=返回类型
 *
 * 说明：request 直接返回 async 函数结果，不使用 new Promise(async ...) 执行器反模式——
 * 执行器内 await 的拒绝不会被 Promise 构造函数捕获，会导致外层 Promise 永久悬挂，
 * 且拒绝变成未处理异常（unhandled rejection）。
 */
class FetchService<T = Response> {
  ...
  public async request(requestParams: RequestParams): Promise<T> {
    ...
    if (this.requestInterceptor) {
      ...
      if (!newRequestParams) {
        throw new Error('Request cancelled by interceptor')
      }
