...
import type { PromotionPlan } from '@web/api/materials/material.types'
import { apiGetMaterialGroupList } from '@web/api/materials/material.api'
    ...
    const syncTabs = async (preferredPlanId?: string | null) => {
      ...
      try {
        ...
        if (validPreferredPlanId) {
          ...
          // 没有有效选中项时落到默认素材组，而不是列表第一项：
          // 任一新建议题组都可能把首屏顶成一个空组，用户看到的就是「内容创作里什么都没有」。
          validSelectedId = (list.find(plan => plan.isDefault) ?? list[0]).id
        }
