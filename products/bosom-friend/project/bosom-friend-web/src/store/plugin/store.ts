...
import type { CreateChannelAccountParams, SocialAccount } from '@web/api/accounts/account.types'
import type { ChannelCreatePublishFlowParams } from '@web/api/channels/channel.types'
import type { IPubParams } from '@web/components/PublishDialog/publishDialog.type'
import dayjs from 'dayjs'
import utc from 'dayjs/plugin/utc'
import lodash from 'lodash'
import { create } from 'zustand'
import { combine } from 'zustand/middleware'
import { createChannelAccountApi } from '@web/api/accounts/account.api'
import { createChannelPublishFlowApi } from '@web/api/channels/channel.api'
import { PlatType } from '@web/app/config/platConfig'
import { directTrans } from '@web/app/i18n/client'
import { buildChannelPublishFlowParams, getPublishRecordIdFromFlow, isPublishTitleSupported } from '@web/components/PublishDialog/PublishDialog.util'
import { PluginVersionLast } from '@web/constant'
import { useAccountStore } from '@web/store/account'
import { WEB_API_BASE_URL } from '@web/config/api'
import { isPlatformEnabledSync } from '@web/store/platformMetadata'
...
const ERROR_MESSAGES = {
      PLUGIN_NOT_INSTALLED: '请先安装 Bosom Friend 浏览器插件',
...
function getPluginApiBaseUrl() {
  const apiBaseUrl = WEB_API_BASE_URL
  if (!apiBaseUrl)
    return undefined
...
async function startWxSphLinkPolling(params: {
  ...
  if (!anchor || !window.ZhiyinPlugin?.wxSphStartLinkPolling)
    return

  const token = useUserStore.getState().token
  const apiBaseUrl = getPluginApiBaseUrl()
  try {
    const response = await window.ZhiyinPlugin.wxSphStartLinkPolling({
    ...
    console.error('视频号链接轮询启动失败：', error)
  }
}
  ...
  combine({ ...getStore() }, (set, get) => {
    const methods = {
      ...
      async fetchPluginVersion(force = false) {
        const plugin = typeof window !== 'undefined' ? window.ZhiyinPlugin : undefined
        const isInstalled = hasPluginApi()
      ...
      /** 检查插件是否安装 */
      checkPlugin() {
        const isAvailable = hasPluginApi()
      ...
      async checkPermission() {
        ...
        try {
          const result = await window.ZhiyinPlugin!.checkPermission()
      ...
      async refreshAllPlatformAccounts(options?: RefreshPlatformAccountsOptions) {
        ...
        await Promise.all(
          PLUGIN_SUPPORTED_PLATFORMS.map(async (platform) => {
            try {
              accounts[platform] = await window.ZhiyinPlugin!.login(platform)
            }
            catch {
      ...
      async login(platform: PluginPlatformType) {
        ...
        try {
          const result = await window.ZhiyinPlugin!.login(platform)
          set({
      ...
      async publish(params: PublishParams, onProgress?: ProgressCallback) {
        ...
        try {
          const result = await window.ZhiyinPlugin!.publish(params, (progress) => {
            // 更新该账号的进度
