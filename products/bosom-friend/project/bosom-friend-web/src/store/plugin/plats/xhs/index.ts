...
class XhsPlatformInteraction implements IPlatformInteraction {
  ...
  async likeWork(workId: string, isLike: boolean): Promise<LikeResult> {
    ...
    if (window.ZhiyinPlugin!.unifiedInteraction) {
      const response = await window.ZhiyinPlugin!.unifiedInteraction({
        platform: 'xhs',
        action: 'like',
    ...
    const response = await window.ZhiyinPlugin!.xhsRequest<XhsBaseResponse>({
      path,
      method: 'POST',
  ...
  async commentWork(params: CommentParams): Promise<CommentResult> {
    ...
    if (!params.replyToCommentId && window.ZhiyinPlugin!.unifiedInteraction) {
      const response = await window.ZhiyinPlugin!.unifiedInteraction({
        platform: 'xhs',
        action: 'comment',
    ...
    const response = await window.ZhiyinPlugin!.xhsRequest<XhsCommentResponse>({
