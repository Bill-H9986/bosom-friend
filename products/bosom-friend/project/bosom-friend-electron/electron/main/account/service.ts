...
export class AccountService {
  ...
  async checkAccountLoginCore(pType: PlatType, uid: string) {
    ...
    // 官方插件式平台：共享分区会话有效即在线（平台 API 不通/风控时不误标离线）
    if (
      pType === PlatType.Douyin
      || pType === PlatType.Xhs
      || pType === PlatType.WxSph
    ) {
      const sessionValid = await isPlatformSessionValid(pType);
      if (sessionValid) {
        // 会话有效即在线；仍尝试 API 刷新最新账号信息（粉丝/昵称），失败保留旧值
        const fresh = await platController
          .platLoginCheck(pType, accountInfo)
          .catch(() => ({ online: true, account: undefined }));
        await this.updateAccountInfo(accountInfo.id, {
          status: AccountStatus.USABLE,
          ...(fresh && typeof fresh.account === 'object' ? fresh.account : {}),
        });
        return await this.getAccountById(accountInfo.id);
      }
    }
  ...
  async addOrUpdateAccount(
    ...
    // 同平台换号登录（uid 变化）：更新旧行为新账号，避免残留失效账号行。
    // 旧 uid 为合成占位值（如 xhs_<时间戳>）时保持原 uid，避免每次同步都生成新合成 uid 造成行内数据抖动
    if (!accountData && query.uid) {
      const existingByPlatform = await this.accountRepository.findOne({
        where: { userId: query.userId, type: query.type },
      });
      if (existingByPlatform && existingByPlatform.uid && existingByPlatform.uid !== query.uid) {
        const nextUid = existingByPlatform.uid.startsWith(`${query.type}_`)
          ? existingByPlatform.uid
          : query.uid;
        await this.accountRepository.update({ id: existingByPlatform.id }, {
          ...account,
          uid: nextUid,
        });
        return {
