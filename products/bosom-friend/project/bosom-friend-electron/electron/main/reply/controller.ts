...
export class ReplyController {
  ...
  /** 全局评论接待：立即轮询一轮（全局监控页手动触发） */
  @Icp('ICP_REPLY_POLL_NOW')
  async pollNow() {
    return this.replyService.pollAllCommentTasks();
  }

  /** 全局评论接待监控状态（全局监控页实时展示） */
  @Icp('ICP_REPLY_MONITOR_STATUS')
  async monitorStatus() {
    return this.replyService.getMonitorStatus();
  }
  ...
  async replyComment(
    ...
    const res = await platController.replyCommentLegacy(
      account,
      commentId,
      content,
      option,
