...
import { generateABogus } from '../../main/dm/abogus.cjs';
import {
...
export class DouyinService {
  ...
  /** 登录窗口关闭时的回调：用于立即结束 filterCookie 等待，避免用户关闭窗口后登录流程挂起 */
  private windowCloseHandlers: { [key: number]: () => void } = {};

  /**
   * 取消登录：关闭授权窗口并结束等待（窗口销毁会触发 close handler 立即 reject）
   */
  public cancelLogin(): void {
    Object.values(this.windowMap).forEach((win) => {
      if (win && !win.isDestroyed()) {
        win.destroy();
      }
    });
    Object.keys(this.windowMap).forEach((key) => {
      delete this.windowMap[key as unknown as number];
    });
  }
  ...
  async loginOrView(
    ...
    return new Promise((resolve, reject) => {
      logger.info('[douyin-login] loginOrView 开始, model=', authModel);
      this.createAuthorizationWindow(
        ...
        try {
          ...
          // 私钥及 web_protect 非必需：获取失败不阻断登录（cookie 才是关键）
          let localStorage = '';
          try {
            localStorage = await this.filterLocalStorage(winContentsId);
          }
          catch (localStorageError) {
            logger.warn('获取 localStorage 失败，继续登录流程:', localStorageError);
          }
          // 用户信息非必需：获取失败用空对象兜底，后续可再补
          let userInfo: any = {};
          try {
            userInfo = await this.getUserInfo(cookies);
          }
          catch (userInfoError) {
            logger.warn('获取用户信息失败，继续登录流程:', userInfoError);
          }
        ...
        .catch((err) => {
          // 授权窗口创建/加载失败必须向外传播，否则登录流程会永远挂起
          logger.error('创建授权窗口失败:', err);
          reject(err);
        });
    });
  }
  ...
  async checkLoginStatus(cookies: string): Promise<boolean> {
    try {
      const res = await this.fetchUserInfo(CommonUtils.convertCookieToJson(cookies));
  ...
  /**
   * 用 Node fetch 请求创作者用户信息接口（替代 net.request）。
   * net.request 携带旧 Cookie 时存在原生崩溃风险，fetch 只抛 JS 异常，可安全兜底。
   */
  private async fetchUserInfo(cookieString: string): Promise<any> {
  ...
  async publishVideoWorkApi(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        logger.info('抖音视频发布最终参数：', publishVideoParams);
        // 发布视频
        const headers = {
          Cookie: cookieString,
        ...
        if (publishResult.status === 403 || publishResult.data === null) {
          logger.error(`发布失败，状态码403或返回数据为空`);
        ...
        // 统一 dataId 口径：优先明文 aweme_id（作品列表/管理页/评论驱动均用明文），
        // 加密 item_id 只作为兜底，避免发布记录与回读链路对不上
        const item_id =
          publishResult.data.aweme?.aweme_id ||
          publishResult.data.item_id_plain ||
        ...
        logger.info(`抖音视频发布成功，返回数据:`, response);
        resolve(response);
      } catch (err) {
        logger.error('发布视频过程中出现错误:', err);
        callback(-1);
        reject(err);
      }
    });
  ...
  async publishImageWorkApi(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        logger.info('抖音图文发布最终参数：', publishImgParams);
        // 发布图文
        ...
        if (
          ...
          logger.error(`发布失败，状态码异常:`, publishResult.status_code);
        ...
        logger.info(`发布成功，返回数据:`, response);
        resolve(response);
      } catch (err) {
        logger.error(`发布图片作品失败:`, err);
        reject(err);
      }
    });
  }
  ...
  private async createAuthorizationWindow(cookies: any = null) {
    return new Promise<{ winContentsId: number; partition: string }>(
      async (resolve) => {
        logger.info('[douyin-login] createAuthorizationWindow 开始');
        // 生成随机partition
        ...
        if (cookies) {
          try {
            ...
            logger.error('Set cookies error:', err);
          }
        }
        ...
        win.loadURL(this.loginUrl, {
          ...
          logger.error('登录页加载失败:', err);
        });

        // 加载失败诊断：网络/代理异常时记录错误码，避免登录流程无提示挂起
        win.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
          logger.error('登录页加载失败:', { errorCode, errorDescription, validatedURL });
        });
        ...
        // 监听窗口销毁：立即结束 filterCookie 等待（用户手动关闭登录窗口 = 取消登录）
        win.webContents.on('destroyed', () => {
          ...
          if (this.windowCloseHandlers.hasOwnProperty(winContentsId)) {
            this.windowCloseHandlers[winContentsId]();
            delete this.windowCloseHandlers[winContentsId];
          }
          delete this.windowMap[winContentsId];
        });
  ...
  private async filterCookie(
    ...
    return new Promise((resolve, reject) => {
      let attempts = 0;
      // 安全 cookie 可能延迟生成：登录 cookie 已出现但安全 cookie 未就绪时，再等 60 秒
      let secUidGrace = 0;
      const maxAttempts = 100; // 最大等待 100 轮 * 3 秒 = 300 秒
      const secUidGraceMax = 20;
      ...
      // 用户手动关闭登录窗口 = 取消登录：立即结束等待，避免挂起 300 秒
      this.windowCloseHandlers[winContentsId] = () => {
        cleanup();
        reject('登录窗口已关闭');
      };
      ...
      this.cookieIntervalList[winContentsId] = setInterval(async () => {
        try {
          ...
          else if (alreadyLogin && !hasSecUid) {
            // 安全 cookie 可能稍后才生成，最多再等 60 秒，到期后允许登录流程继续
            secUidGrace++;
            if (secUidGrace >= secUidGraceMax) {
              cleanup();
              resolve(cookies);
            }
          }
  ...
  private async filterLocalStorage(winContentsId: number): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // 如果仍然找不到窗口
        if (!win || win.isDestroyed()) {
          logger.error('找不到窗口或窗口已销毁, ID:', winContentsId);
          reject('找不到有效的窗口');
          return;
        }
        ...
        while (retryCount < maxRetries) {
          if (!win || win.isDestroyed()) {
            logger.error('窗口已被销毁，停止获取 localStorage');
            reject('窗口已被销毁');
            break;
          }
              ...
              const privateKey = await win.webContents.executeJavaScript(`
                (function() {
                  try {
                    ...
                    logger.error('解析 privateKey 失败:', e);
                    return null;
                  }
                })()
              `);
              ...
              const webProtect = await win.webContents.executeJavaScript(`
                (function() {
                  try {
                    ...
                    logger.error('解析 webProtect 失败:', e);
                    return null;
                  }
                })()
              `);
        ...
        logger.error('获取 localStorage 过程出错:', err);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          logger.error('获取用户Uid失败:', userInfoRes.status_msg);
  ...
  private async uploadVideo(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        logger.info('分片合并结果:', uploadFinishRes);
        ...
        logger.info('视频上传成功，视频ID:', commitRes.Result.Results[0].Vid);
        resolve(commitRes.Result.Results[0].Vid);
      } catch (err) {
        logger.error('视频上传过程出错:', err);
        let errorMessage;
        // @ts-ignore
        if (err && err.message) {
          // @ts-ignore
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!headers['x-ware-csrf-token']) {
          logger.error('获取CSRF_TOKEN失败，响应头中没有x-ware-csrf-token');
          reject('获取CSRF_TOKEN失败,请稍后重试!');
          return;
        }
        ...
        logger.error('获取CSRF Token过程出错:', err);
        reject(err);
      }
    });
  }
  ...
  private async getBdTicketHeaders(tokens: any): Promise<any> {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        logger.info('bdRes：', bdRes);

        resolve(bdRes);
      } catch (err) {
        logger.error('获取bd-Ticket失败:', err);
        reject(err);
      }
    });
  }
  ...
  async getSearchNodeList(
    ...
    logger.info('thisUrithisUrithisUri', thisUri);
    // 方法
  ...
  async getCreatorCommentList(
    ...
    // creator 版评论接口已升级要求新签名（旧接口失效），改用网页版接口（带 a_bogus 签名）
    const cookieString = CommonUtils.convertCookieToJson(cookie);
    const ua =
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.6723.191 Electron/33.4.11 Safari/537.36';
    const params = `device_platform=webapp&aid=6383&channel=channel_pc_web&aweme_id=${item_id}&cursor=${pageInfo.cursor || 0}&count=${pageInfo.count || 20}&sort=0`;
    const aBogus = generateABogus(params, ua);
    const url = `https://www.douyin.com/aweme/v1/web/comment/list/?${params}&a_bogus=${encodeURIComponent(aBogus)}`;
    ...
    // 转换为 creator 兼容的原始格式（上层 Douyin.getCommentList 依赖 user_info/comment_id/text）
    const rawComments = (data.comments || []).map((v: any) => {
      const subList: any[] = [];
      // web 评论接口的回复字段是 reply_comment（sub_comments 为旧字段，始终为空）
      const sub = (v.reply_comment || v.sub_comments || []) as any[];
      ...
      return {
        user_info: {
          user_id: v.user?.uid || '',
          screen_name: v.user?.nickname || '',
          avatar_url: v.user?.avatar_thumb?.url_list?.[0] || '',
        },
        comment_id: String(v.cid),
        text: v.text || '',
        digg_count: String(v.digg_count || 0),
        reply_count: String((v.reply_comment || []).length || 0),
        level: 1,
        sub_comment_list: sub,
      };
    });
    // 同时提供转换后的 list 结构
    const comments = rawComments.map((v: any) => ({
      userId: v.user_info.user_id,
      dataId: item_id,
      commentId: v.comment_id,
      content: v.text,
      likeCount: Number.parseInt(v.digg_count) || 0,
      nikeName: v.user_info.screen_name,
      headUrl: v.user_info.avatar_url,
      data: v,
      subCommentList: v.sub_comment_list,
    }));
    // 兼容上层调用：同时提供 data.comment_info_list 结构与 list/pageInfo 结构
    return {
      data: {
        comment_info_list: rawComments,
  ...
  async getCreatorCommentListByOther(
    ...
    // // a_bogus=mjsjDq7jDpAcFdFb8KEfC5Fl6g6ArTSyNeidWSaTyPY4T1UTpbPUNPb9GxwoA1vPFRBhhH-73VM%2FbDdbO0UwZo9pwmkvuKiRz02C9zmoMHZ3TTv2XNWsCvSELiPTUCsYY%2FA9i2RRXs0KId5WnH9iAp17u%2FvrmRfdMH-XV2TjE9um0ASjhx%2FIa5JBxhwqjD%3D%3D
        ...
        // 使用创作者后台评论列表接口（登录态可用，无需 a_bogus 签名）
    const res = await requestNet<DouyinCreatorCommentListResponse>({
      // 注意：加密 item_id 包含 @ 和 =，不能经过 URL 编码，直接拼接
      url: `https://creator.douyin.com/aweme/v1/creator/comment/list/?item_id=${item_id}&cursor=${pageInfo.cursor || '0'}&count=${pageInfo.count || 20}`,
      headers: {
        cookie: CookieToString(cookie),
      },
      method: 'GET',
  ...
  private async postFormData(
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        // 检查HTTP状态码
        if (!response.ok) {
          logger.error(`HTTP错误状态码: ${response.status}`);
        ...
        try {
          ...
          if (!result) {
            logger.error(`解析后的数据为空`);
            throw new Error('解析后的数据为空');
          }

          return result;
        } catch (err) {
          logger.error(`解析响应数据失败:`, err);
          logger.error(`导致错误的原始数据:`, responseText);
          throw new Error(
  ...
  async creatorCommentReplyOther(cookie: Electron.Cookie[], data: any) {
    logger.info('wentipaicha1');
    const thisUri = `https://www.douyin.com/aweme/v1/web/comment/publish/?${jsonToQueryString(
      {
        aid: '6383',
      },
    )}`;
    logger.info('wentipaicha2');
    const cookieString = CommonUtils.convertCookieToJson(cookie);
    logger.info('wentipaicha3');
    const csrfToken = await this.getSecsdkCsrfToken(cookieString);
    logger.info('wentipaicha4');
    ...
    logger.info('douyin index ------ res', res);
    return res;
  }
