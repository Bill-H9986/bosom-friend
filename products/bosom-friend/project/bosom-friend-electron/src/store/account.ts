  ...
  combine(
    ...
    (set, get, storeApi) => {
      const methods = {
        ...
        // 轮询检测账户有效性（带失败退避：全离线时拉长间隔，避免对平台接口高频调用触发风控）
          ...
          return new Promise(async () => {
            while (true) {
              ...
              if (accountList.length > 0) {
                const results = await acpAccountLoginCheckMulti(
                  accountList.map((v) => ({
                    pType: v.type,
                    uid: v.uid,
                  })),
                );
                allOffline = results.every(item => !item || item.status !== AccountStatus.USABLE);
              }

              // 全部离线时退避到 30 分钟；有在线账号时保持 10 分钟检测频率
              await sleep((allOffline ? 30 : 10) * 60 * 1000);
            }
          });
        },
      };
