...
function encodeFields(fs){fs.sort((a,b)=>a.num-b.num);const c=[];for(const f of fs){c.push(writeVarint((BigInt(f.num)<<3n)|BigInt(f.wt)));if(f.wt===0)c.push(writeVarint(f.v));else if(f.wt===2){c.push(writeVarint(f.v.length));c.push(f.v);}else if(f.wt===1||f.wt===5)c.push(f.v);}return Buffer.concat(c);}
...
async function genMsToken(){
  ...
  const m=tokenSrc.match(/new TokenConfig\(\s*"([^"]+)",\s*(-?\d+),\s*(\d+),\s*(\d+),\s*"([^"]+)"/);
  const res=await fetch(m[1],{method:'POST',headers:{'User-Agent':UA,'Content-Type':'application/json'},body:JSON.stringify({magic:Number(m[2]),version:Number(m[3]),dataType:Number(m[4]),strData:m[5],tspFromClient:Date.now()})});
  const setCookie=res.headers.getSetCookie?res.headers.getSetCookie():[];
  for(const c of setCookie){const x=c.match(/msToken=([^;]+)/);if(x)return x[1];}
  return null;
}
(async()=>{
  const msToken=await genMsToken();
  const verifyFp='verify_meb5wlxw_ug0uJNeW_hGev_4QmL_9kh9_sBxHCmY5R37Y';
  const p='msToken='+msToken+'&verifyFp='+verifyFp+'&fp='+verifyFp;
  const aBogus=generateABogus(p,UA);
  ...
  upd(root,[8,100],(cf)=>{setStr(cf,1,'0:1:98478746276:744786308115968');setVar(cf,2,1);setVar(cf,3,'7672290885698159141');setStr(cf,4,JSON.stringify({mention_users:[],aweType:700,richTextInfos:[],text:'错误解码测试'}));setStr(cf,8,crypto.randomUUID());});
  const body=encodeFields(root);
  const res=await fetch('https://imapi.douyin.com/v1/message/send?'+p+'&a_bogus='+encodeURIComponent(aBogus),{method:'POST',headers:{Cookie:'sessionid='+sessionid+'; sessionid_ss='+sessionid+';',accept:'application/x-protobuf','content-type':'application/x-protobuf',origin:'https://www.douyin.com',referer:'https://www.douyin.com/','user-agent':UA,'accept-language':'zh-CN,zh;q=0.9','sec-ch-ua':'"Not;A=Brand";v="99", "Google Chrome";v="139", "Chromium";v="139"','sec-ch-ua-mobile':'?0','sec-ch-ua-platform':'"macOS"','sec-fetch-dest':'empty','sec-fetch-mode':'cors','sec-fetch-site':'same-site'},body});
  const buf=Buffer.from(await res.arrayBuffer());
  console.log('HTTP',res.status,'HEX',buf.toString('hex'));
  // try to find readable text in the protobuf
  const text=buf.toString('latin1');
  const matches=[...text.matchAll(/[\x20-\x7e\u4e00-\u9fa5]{4,}/g)].map(m=>m[0]);
  console.log('TEXT_PARTS',JSON.stringify(matches.slice(0,10)));
})();
