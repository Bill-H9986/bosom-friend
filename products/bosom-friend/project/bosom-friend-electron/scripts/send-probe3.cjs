...
(async()=>{
  ...
  upd(root,[8,100],(cf)=>{setStr(cf,1,'0:1:98478746276:744786308115968');setVar(cf,2,1);setVar(cf,3,'7672290885698159141');setStr(cf,4,JSON.stringify({mention_users:[],aweType:700,richTextInfos:[],text:'链路诊断三，请忽略。'}));setStr(cf,8,crypto.randomUUID());});
  const body=encodeFields(root);
  const query='msToken='+encodeURIComponent(freshMsToken)+'&verifyFp=verify_meb5wlxw_ug0uJNeW_hGev_4QmL_9kh9_sBxHCmY5R37Y&fp=verify_meb5wlxw_ug0uJNeW_hGev_4QmL_9kh9_sBxHCmY5R37Y';
  ...
  console.log('TEXT',buf.toString('utf8').slice(0,250));
})();
